...
START_TEST(univ_person_new_name_is_saved) {
  univ_person* p = univ_person_new("Name",
				   UNIV_PERSON_SERVICE_STUFF);
  
  char* person_name = univ_person_get_name(p, NULL);
...
START_TEST(univ_person_get_name_respects_non_null_dest){
  ...
  char* res = univ_person_get_name(p, person_name);
...
Suite* univ_person_suite(){
  ...
  tcase_add_test(tc_core,
		 univ_person_new_name_is_saved);
  tcase_add_test(tc_core,
		 univ_person_new_category_is_saved);
  tcase_add_test(tc_core,
		 univ_person_get_name_respects_non_null_dest);
  tcase_add_test(tc_core,
		 univ_person_new_NULL_name_error);
