...
#define ok_return(ret) univ_person_status_ = UNIV_PERSON_S_OK;\
                       return ret;
...
char* univ_person_get_name(univ_person* p, char* dest){
  if(dest == NULL) dest = malloc(strlen(p->name)+1);
  strcpy(dest, p->name);
  ok_return(dest);
}
...
univ_person_status univ_person_get_error(){
  return univ_person_status_;
}
